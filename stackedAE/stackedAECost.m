...
z4 = stack{3}.w * a3 + repmat(stack{3}.b, 1, size(a3, 2));
a4 = sigmoid(z4);

z5 = bsxfun(@minus, softmaxTheta * a4, max(softmaxTheta * a4));
hx = bsxfun(@rdivide, exp(z5), sum(exp(z5)));

cost = -1 * sum(sum(groundTruth .* log(hx))) / M + lambda * sum(sum(softmaxTheta.^2)) / 2;
softmaxThetaGrad = -1 * (groundTruth - hx) * a4' / M + lambda * softmaxTheta;

delta4 = (-softmaxTheta' * (groundTruth - hx)) .* a4 .* (1 - a4);
